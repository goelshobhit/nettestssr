...
// data2.directLibraryParent_html.fields.title
...
function renderMenu(libMenu, subItemsList, subItemsList1, openMenu, setOpenMenu) {
  ...
  function getRenderSubItems2(title1) {
    const itemsOfSubMenu = filter(subItemsList1, item => item.directLibraryParent_html.fields.title === title1);
    return (
      <Menu key={title1}>
        {map(hasSubMenu(itemsOfSubMenu, subItemsList1), item => (
          <SubMenu key={item.title} expandIcon={item.hasSubMenu ? <i className="ant-menu-submenu-arrow" /> : <span />}>
            <Link
  ...
  function getRenderSubItems(title1) {
    const itemsOfSubMenu = filter(subItemsList, item => item.directLibraryParent_html.fields.title === title1);
